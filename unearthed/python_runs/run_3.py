...
async def mission_N_raise_the_ship(task_id):
    ...
    try:
        ...
        await move_forward(30, speed=(int)(DEFAULT_MOVEMENT_SPEED*0.66)) # 2/3 speed so that we ran with power
...
async def mission_N_raise_payload_push_it_multiple_times(task_id):
    ...
    try:
        ...
        # Lower the arm
        await move_left_motor(270) # drop it down
        for i in range(4):            
            await move_forward(10)
            await move_backward(10)
        # return to base
        await move_backward(30)
        loop.remove_task(task_id)
...
async def run3_mN_mN(task_id):
    ...
    try:
        ...
        await mission_N_raise_the_ship('mission_raise_the_ship')
        # Small delay between missions
        await asyncio.sleep_ms(500)
        # Mission 2: Move the hook
        await mission_N_raise_payload_push_it_multiple_times('mission_raise_payload')
        # Small delay between missions
        await asyncio.sleep_ms(500)
...
async def mainLoop():
    ...
    loop.add_main_task("main_task", events.when_program_starts('run_3', run3_mN_mN))
