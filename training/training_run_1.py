...
async def mission_arm_drop(task_id):
    ...
    try:
        ...
        # Move arm motor 750 degrees to the left using helper function
        await turn_left(45)
        await move_left_motor_sideways(-750)
        await move_left_motor_sideways(750)
        loop.remove_task(task_id)
...
async def mission_forward_and_back(task_id):
    """
    Mission: Move robot in a square pattern with precise 90-degree turns
    ...
    try:
        await move_forward(20, speed=500)
        await move_backward(20, speed=200)
        loop.remove_task(task_id)
...
async def simple_run(task_id):
    ...
    try:
        ...
        await mission_forward_10cm('mission_1')
        # Small delay between missions
        await asyncio.sleep_ms(500)
        # Mission 2: Move arm 90 degrees using mission function
        await mission_arm_drop('mission_2')
        # Small delay between missions
        await asyncio.sleep_ms(500)
        # Mission 3: Move in square pattern using mission function
        await mission_forward_and_back('mission_3')
        loop.remove_task(task_id)
    except asyncio.CancelledError:
        when_cancelled()
...
async def mainLoop():
    ...
    loop.add_main_task("main_task", events.when_program_starts('simple_run', simple_run))
